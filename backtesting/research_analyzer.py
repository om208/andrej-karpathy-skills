...
class StrategyResearchAnalyzer:
    ...
    def __init__(self, trades_df):
        ...
        # Detect which PnL column is used
        self.pnl_col = 'total_pnl' if 'total_pnl' in trades_df.columns else 'pnl'
        # Detect which market move column is used
        self.move_col = 'positive_move_after_159min' if 'positive_move_after_159min' in trades_df.columns else 'drastic_move_percent'
        # Detect which exit_reason column is used
        self.exit_reason_col = 'lot1_exit_reason' if 'lot1_exit_reason' in trades_df.columns else 'exit_reason'
    ...
    def _analyze_winning_trades(self):
        ...
        facts = {
            ...
            'total_pnl': f"${wt[self.pnl_col].sum():.2f}",
            'avg_pnl': f"${wt[self.pnl_col].mean():.2f}",
            'median_pnl': f"${wt[self.pnl_col].median():.2f}",
            'max_pnl': f"${wt[self.pnl_col].max():.2f}",
            'min_pnl': f"${wt[self.pnl_col].min():.2f}",
            'std_dev_pnl': f"${wt[self.pnl_col].std():.2f}",

            'avg_minutes_held': f"{wt['minutes_held'].mean():.0f} min",
            'median_minutes_held': f"{wt['minutes_held'].median():.0f} min",

            'exit_reason_breakdown': wt[self.exit_reason_col].value_counts().to_dict() if 'lot1_exit_reason' in wt.columns else wt[self.exit_reason_col] if 'lot1_exit_reason' in self.trades_df.columns else self.trades_df.get('exit_reason', pd.Series([])).value_counts().to_dict() if 'exit_reason' in wt.columns else {},

            'avg_inside_bar_range': f"${wt['inside_bar_range'].mean():.2f}",
            'avg_prev_bar_range': f"${wt['prev_bar_range'].mean():.2f}",

            'avg_drastic_move': f"{wt[self.move_col].mean():.2f}%" if 'positive_move_after_159min' in wt.columns and wt[self.move_col].notna().any() else f"{wt.get('drastic_move_percent', pd.Series([0])).mean():.2f}%",
            'max_drastic_move': f"{wt[self.move_col].max():.2f}%" if 'positive_move_after_159min' in wt.columns and wt[self.move_col].notna().any() else f"{wt.get('drastic_move_percent', pd.Series([0])).max():.2f}%",
    ...
    def _analyze_losing_trades(self):
        ...
        facts = {
            ...
            'total_loss': f"-${abs(lt[self.pnl_col].sum()):.2f}",
            'avg_loss': f"-${abs(lt[self.pnl_col].mean()):.2f}",
            'median_loss': f"-${abs(lt[self.pnl_col].median()):.2f}",
            'max_loss': f"-${abs(lt[self.pnl_col].min()):.2f}",
            'min_loss': f"-${abs(lt[self.pnl_col].max()):.2f}",
            'std_dev_loss': f"${lt[self.pnl_col].std():.2f}",

            'avg_minutes_held': f"{lt['minutes_held'].mean():.0f} min",
            'median_minutes_held': f"{lt['minutes_held'].median():.0f} min",

            'exit_reason_breakdown': lt[self.exit_reason_col].value_counts().to_dict() if 'lot1_exit_reason' in lt.columns else lt[self.exit_reason_col] if 'lot1_exit_reason' in self.trades_df.columns else self.trades_df.get('exit_reason', pd.Series([])).value_counts().to_dict() if 'exit_reason' in lt.columns else {},

            'avg_inside_bar_range': f"${lt['inside_bar_range'].mean():.2f}",
            'avg_prev_bar_range': f"${lt['prev_bar_range'].mean():.2f}",

            'avg_drastic_move': f"{lt.get(self.move_col, pd.Series([0])).mean():.2f}%",
            'max_drastic_move': f"{lt.get(self.move_col, pd.Series([0])).max():.2f}%",
    ...
    def _analyze_patterns(self):
        ...
        facts['inside_bar_analysis'] = {
            'small_bars_(<$50)': {
                ...
                'avg_pnl': f"${small_inside_bars[self.pnl_col].mean():.2f}" if len(small_inside_bars) > 0 else "N/A"
            },
            'medium_bars_($50-$100)': {
                ...
                'avg_pnl': f"${medium_inside_bars[self.pnl_col].mean():.2f}" if len(medium_inside_bars) > 0 else "N/A"
            },
            'large_bars_(>$100)': {
                ...
                'avg_pnl': f"${large_inside_bars[self.pnl_col].mean():.2f}" if len(large_inside_bars) > 0 else "N/A"
            }
        }
        ...
        facts['bar_ratio_analysis'] = {
            'very_small_ratio_(<0.5)': {
                ...
                'avg_pnl': f"${ratio_lt_0_5[self.pnl_col].mean():.2f}" if len(ratio_lt_0_5) > 0 else "N/A"
            },
            'medium_ratio_(0.5-0.8)': {
                ...
                'avg_pnl': f"${ratio_0_5_to_0_8[self.pnl_col].mean():.2f}" if len(ratio_0_5_to_0_8) > 0 else "N/A"
            },
            'high_ratio_(>=0.8)': {
                ...
                'avg_pnl': f"${ratio_gte_0_8[self.pnl_col].mean():.2f}" if len(ratio_gte_0_8) > 0 else "N/A"
            }
        }
    ...
    def _analyze_timing(self):
        ...
        facts['quick_exits_(<60_min)'] = {
            ...
            'avg_pnl': f"${quick_exits[self.pnl_col].mean():.2f}" if len(quick_exits) > 0 else "N/A",
        ...
        facts['medium_holds_(60-159_min)'] = {
            ...
            'avg_pnl': f"${medium_holds[self.pnl_col].mean():.2f}" if len(medium_holds) > 0 else "N/A",
        ...
        facts['long_holds_(>=159_min)'] = {
            ...
            'avg_pnl': f"${long_holds[self.pnl_col].mean():.2f}" if len(long_holds) > 0 else "N/A",
    ...
    def _analyze_volatility(self):
        ...
        facts['low_volatility_entries'] = {
            ...
            'avg_pnl': f"${low_vol[self.pnl_col].mean():.2f}" if len(low_vol) > 0 else "N/A",
        ...
        facts['medium_volatility_entries'] = {
            ...
            'avg_pnl': f"${mid_vol[self.pnl_col].mean():.2f}" if len(mid_vol) > 0 else "N/A",
        ...
        facts['high_volatility_entries'] = {
            ...
            'avg_pnl': f"${high_vol[self.pnl_col].mean():.2f}" if len(high_vol) > 0 else "N/A",
    ...
    def _compare_wins_vs_losses(self):
        ...
        if len(self.winning_trades) > 0 and len(self.losing_trades) > 0:
            facts['win_vs_loss_comparison'] = {
                'avg_pnl_winners': f"${self.winning_trades[self.pnl_col].mean():.2f}",
                'avg_pnl_losers': f"-${abs(self.losing_trades[self.pnl_col].mean()):.2f}",
                'pnl_difference': f"${self.winning_trades[self.pnl_col].mean() + abs(self.losing_trades[self.pnl_col].mean()):.2f}",
                ...
                'avg_drastic_move_winners': f"{self.winning_trades[self.move_col].mean():.2f}%",
                'avg_drastic_move_losers': f"{self.losing_trades[self.move_col].mean():.2f}%",

                'exit_reason_winners': dict(self.winning_trades[self.exit_reason_col] if 'lot1_exit_reason' in self.trades_df.columns else self.trades_df.get('exit_reason', pd.Series([])).value_counts()),
                'exit_reason_losers': dict(self.losing_trades[self.exit_reason_col] if 'lot1_exit_reason' in self.trades_df.columns else self.trades_df.get('exit_reason', pd.Series([])).value_counts())
            }
    ...
    def _analyze_drastic_moves(self):
        ...
        if len(past_159) > 0:
            facts['trades_held_past_159_minutes'] = {
                ...
                'avg_pnl': f"${past_159[self.pnl_col].mean():.2f}",
                'avg_drastic_move': f"{past_159[self.move_col].mean():.2f}%",
                'max_drastic_move': f"{past_159[self.move_col].max():.2f}%",
                'min_drastic_move': f"{past_159[self.move_col].min():.2f}%"
            }
        ...
        if len(early_exits) > 0:
            facts['early_exits_before_159_minutes'] = {
                ...
                'avg_pnl': f"${early_exits[self.pnl_col].mean():.2f}",
                'avg_drastic_move': f"{early_exits[self.move_col].mean():.2f}%",
                'exit_via_tp': len(early_exits[early_exits[self.exit_reason_col] if 'lot1_exit_reason' in self.trades_df.columns else self.trades_df.get('exit_reason', pd.Series([])) == 'take_profit']),
                'exit_via_sl': len(early_exits[early_exits[self.exit_reason_col] if 'lot1_exit_reason' in self.trades_df.columns else self.trades_df.get('exit_reason', pd.Series([])) == 'stop_loss'])
            }
